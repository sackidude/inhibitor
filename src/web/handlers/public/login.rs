    ...
    match res {
        Some(res) => {
            if res.password == payload.password {
                // TODO! implement hashing on so forth.
                // Generate auth_token
                ...
                let mut auth_cookie =
                    tower_cookies::Cookie::new(token::AUTH_TOKEN, auth_token.to_str());
                ...
                // Construct response
                get_response(LoginResponse::Successfull {
        ...
        None => get_response(LoginResponse::UserNotFound),
    }
}
